...
enum VIDEO_SOURCE_TYPE {
	VIDEO_SOURCE_CAMERA,	    //built-in camera source, set the IVideoSource to NULL
	VIDEO_SOURCE_SCREENCAST, //built-in screen share source, set the IVideoSource to NULL
	VIDEO_SOURCE_EXTERNAL,	//external video source, IVideoSource needed.
};
...
public:
	...
	*
	* @param consumer The {@link IVideoFrameConsumer IVideoFrameConsumer} object which the media engine passes back. You need to reserve this object and pass the video frame to the media engine through this object once the video source is initialized. See the following contents for the definition of {@link IVideoFrameConsumer IVideoFrameConsumer}.
	* @note When initializing the video source, you need to specify a buffer type in the {@link IVideoSource#getBufferType() getBufferType} method and pass the video source in the specified type to the media engine.
	* @return
	...
	*
	* This callback is triggered when the {@link IVideoFrameConsumer IVideoFrameConsumer} is released by the media engine. You can now release the video source as well as the {@link IVideoFrameConsumer IVideoFrameConsumer}.
	*/
	...
	*
	* This callback is triggered when the underlying media engine is ready to start video streaming. You should start the video source to capture the video frame. Once the frame is ready, use the {@link IVideoFrameConsumer IVideoFrameConsumer} to consume the video frame.
	*
	* @return
	*   - true: The external video source is enabled and {@link IVideoFrameConsumer IVideoFrameConsumer} is called to receive video frames.
	...
	* Currently only I420 supported.
	* @return {@link VIDEO_FRAME_TYPE Video buffer type}
	*/
...
public:
  ...
  virtual int updateScreenCaptureRegion(const Rect *rect) = 0;
     ...
     * Same as setVideoSource(VIDEO_SOURCE_EXTERNAL, IVideoSource *source)
     * @param source Sets the video source. See {@link agora.media.IVideoSource IVideoSource}.
     */
    //bool setVideoSource(IVideoSource *source) { return setVideoSource(VIDEO_SOURCE_EXTERNAL, source); }

    /** Customize the video source.
     * Call this method to set a video source into the SDK.
     * @param type The video source type. Specify the source type from 3 types:VIDEO_SOURCE_CAMERA/VIDEO_SOURCE_SCREENCAST/VIDEO_SOURCE_EXTERNAL.
     * @param source Sets the video source. See {@link agora.media.IVideoSource IVideoSource}.
     */
  virtual bool setVideoSource(VIDEO_SOURCE_TYPE type, IVideoSource *source) = 0;
#endif
