...
#define APP_ID				_T("96432b93884e47c79dd14d3c2ad9d6e8")
...
public:
	...
	BOOL SetVideoProfile(VIDEO_PROFILE_TYPE type, bool swap);
	BOOL SetVideoProfileEx(int width, int height, int fps, int bitrate);
	...
	BOOL SetEncoderType(int index);
	BOOL EnableIntraRequest(BOOL bEnable);
	BOOL SetExtentParam(LPCTSTR lpExtendParam);
	CString GetExtentParam(LPCTSTR lpExtendParam);
	BOOL SetFluencyPrecedence(bool fp);

	bool bIntraRequest = false;
	int encoderType = 0;
	VIDEO_PROFILE_TYPE videoProfileType;
	bool bswap = false;
	bool IsJoinChannel(){ return isJoinChannel; }

	void SetLocalMirror(BOOL bEnable);
protected:
	CAgoraObject(void);
