...
BEGIN_MESSAGE_MAP(CVideoDlg, CDialogEx)
	...
	ON_BN_CLICKED((IDC_BTNMODE_VIDEO + 101010), &CVideoDlg::OnBnClickedBtnShowLocal)
...
void CVideoDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	if (!m_bFullScreen){
		...
		// 2»À£¨ Top right corner sub-screen area
	...
	case SCREEN_VIDEO1:
		...
		break;
	case SCREEN_VIDEO2:
		AdjustSizeVideo2(cx, cy);
		AdjustButtonsNormal(cx, cy);
		break;
	case SCREEN_VIDEO3:
		AdjustSizeVideo3(cx, cy);
		AdjustButtonsNormal(cx, cy);
		break;
	case SCREEN_VIDEO4:
		...
		break;
	case SCREEN_VIDEO5:
		AdjustSizeVideo5(cx, cy);
		AdjustButtonsNormal(cx, cy);
		break;
	case SCREEN_VIDEO6:
		AdjustSizeVideo6(cx, cy);
		AdjustButtonsNormal(cx, cy);
		break;
	case SCREEN_VIDEO7:
		AdjustSizeVideo7(cx, cy);
		AdjustButtonsNormal(cx, cy);
		break;
...
{
	if (m_btnShowLocal.GetSafeHwnd() != NULL)
		m_btnShowLocal.MoveWindow(cx / 2 - 370, cy - 48, 72, 36, TRUE);
...
void CVideoDlg::HideAllVideos()
{
	m_wndLocal.ShowWindow(SW_HIDE);

	for (int nIndex = 0; nIndex < MAX_VIDEOS; nIndex++) {
		m_wndVideo[nIndex].ShowWindow(SW_HIDE);
	}
}

void CVideoDlg::ShowVideosWnd()
{
	if (bShowLocalVideo)
		m_wndLocal.ShowWindow(SW_SHOW);
	for (int nIndex = 0; nIndex < showCount - bShowLocalVideo; nIndex++) {
...
{
	HideAllVideos();
	if (m_wndShowVideos[0]->GetSafeHwnd() != NULL)
		m_wndShowVideos[0]->MoveWindow(&m_rcVideoArea, FALSE);
	ShowVideosWnd();
}

void CVideoDlg::AdjustSizeVideo2(int cx, int cy)
{
	HideAllVideos();
	m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height(), FALSE);
	m_wndShowVideos[0]->ShowWindow(SW_SHOW);
	m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height(), FALSE);
	m_wndShowVideos[1]->SetBigShowFlag(FALSE);
	ShowVideosWnd();
}
//1v2
void CVideoDlg::AdjustSizeVideo3(int cx, int cy)
{
	HideAllVideos();
	if (m_nLayout == LAYOUT_AVERAGE){
		m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() / 4, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	}
	else if (m_nLayout == LAYOUT_TEACHER_BIG){
		m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height(), FALSE);
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	}
	ShowVideosWnd();
}
//1v3
void CVideoDlg::AdjustSizeVideo4(int cx, int cy)
{
	...
	m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[2]->MoveWindow(0, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[3]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	ShowButtonsNormal();
	ShowVideosWnd();
}
//1v4
void CVideoDlg::AdjustSizeVideo5(int cx, int cy)
{
	HideAllVideos();
	m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() * 2 / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[3]->MoveWindow(m_rcVideoArea.Width() / 6, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[4]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	
	ShowButtonsNormal();
	ShowVideosWnd();
}
//1v5
void CVideoDlg::AdjustSizeVideo6(int cx, int cy)
{
	HideAllVideos();
	m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() * 2 / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[3]->MoveWindow(0, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[4]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	m_wndShowVideos[5]->MoveWindow(m_rcVideoArea.Width() * 2 / 3, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
	ShowVideosWnd();
}
//1v6
void CVideoDlg::AdjustSizeVideo7(int cx, int cy)
{
	HideAllVideos();
	m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() * 2 / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	m_wndShowVideos[3]->MoveWindow(0, m_rcVideoArea.top + m_rcVideoArea.Height() / 3, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	m_wndShowVideos[4]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top + m_rcVideoArea.Height() / 3, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	m_wndShowVideos[5]->MoveWindow(m_rcVideoArea.Width() * 2 / 3, m_rcVideoArea.top + m_rcVideoArea.Height() / 3, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	m_wndShowVideos[6]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top + m_rcVideoArea.Height() * 2 / 3, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 3, FALSE);
	ShowVideosWnd();
}
...
void CVideoDlg::OnBnClickedBtnclose()
{
	...
	for (int i = 0; i < count && i<MAX_VIDEOS; i++) {
...
void CVideoDlg::SetSetupDlg(CSetupDlg* pDlg)
{
	pDlg->SetParent(this);
	m_pDlgSetup = pDlg;
}
...
void CVideoDlg::OnBnClickedBtnShowLocal()
{
	if (bShowLocalVideo){
		m_btnShowLocal.SetWindowText(_T("Show Local"));
		bShowLocalVideo = !bShowLocalVideo;
		...
		m_btnShowLocal.SetWindowText(_T("Hide Local"));
		bShowLocalVideo = !bShowLocalVideo;
	...
	oldShowLocalVideo = bShowLocalVideo;
}
...
LRESULT CVideoDlg::OnEIDUserJoined(WPARAM wParam, LPARAM lParam)
{
	...
	if (!bFound) {
		...
		memset(&agvWndInfo, 0, sizeof(AGVIDEO_WNDINFO));
		agvWndInfo.nUID = lpData->uid;
		agvWndInfo.nWidth = 0;//lpData->width;
		agvWndInfo.nHeight = 0;//lpData->height;
...
LRESULT CVideoDlg::OnEIDUserOffline(WPARAM wParam, LPARAM lParam)
{
	...
	while (pos != NULL){
		if (m_listWndInfo.GetAt(pos).nUID == lpData->uid) {
			...
			for (int i = index; i < MAX_VIDEOS; i++){
				if (m_wndVideo[i].GetUID() != 0){
					VideoCanvas canvas;
					canvas.uid = m_wndVideo[i].GetUID();
					canvas.view = NULL;
					CAgoraObject::GetAgoraObject()->GetEngine()->setupRemoteVideo(canvas);
				}
			}
			RebindVideoWnd();
			break;
...
LRESULT CVideoDlg::OnRemoteVideoStat(WPARAM wParam, LPARAM lParam)
{
	...
	while (posNext != NULL) {
		...
		if (rWndInfo.nUID == lpData->uid) {
			...
			m_wndVideo[rWndInfo.nIndex].ShowWindow(SW_SHOW);
			//m_wndVideo[rWndInfo.nIndex].SetRemoteInfo(rWndInfo.nBitrate, rWndInfo.nFramerate, rWndInfo.nWidth, rWndInfo.nHeight);
			break;
		}
	}
...
void CVideoDlg::InitCtrls()
{
	...
	GetClientRect(&rcClient);
	m_btnShowLocal.Create(NULL, WS_VISIBLE | WS_CHILD, CRect(0, 0, 1, 1), this, (IDC_BTNMODE_VIDEO+101010));
	...
	m_wndLocal.Create(NULL, NULL, WS_CHILD | WS_VISIBLE | WS_BORDER | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, CRect(0, 0, 1, 1), this, IDC_BASEWND_VIDEO + MAX_VIDEOS);
	...
	m_btnShowLocal.SetFont(&m_ftDes);
	m_btnShowLocal.SetWindowText(_T("Hide Local"));
	m_btnShowLocal.SetTextColor(RGB(0xFF, 0xFF, 0xFF), RGB(0xFF, 0xC8, 0x64), RGB(0xFF, 0xC8, 0x64), RGB(0xCC, 0xCC, 0xCC));
...
void CVideoDlg::ShowVideo1()
{
	...
	if (showCount == 1 && m_wndShowVideos[0]->GetSafeHwnd() != NULL){
		m_wndShowVideos[0]->MoveWindow(&m_rcVideoArea, FALSE);
		m_wndShowVideos[0]->SetBigShowFlag(TRUE);
		m_wndShowVideos[0]->ShowWindow(SW_SHOW);
	}
...
void CVideoDlg::ShowVideo2()
{
	HideAllVideos();
	if (showCount == 2){
		m_wndShowVideos[0]->SetParent(this);
		m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height(), FALSE);
		m_wndShowVideos[0]->ShowWindow(SW_SHOW);
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height(), FALSE);
		m_wndShowVideos[1]->SetBigShowFlag(FALSE);
		m_wndShowVideos[1]->ShowWindow(SW_SHOW);
	}

	m_nScreenMode = SCREEN_VIDEO2;
	ShowVideosWnd();
	ShowButtonsNormal();
}

void CVideoDlg::ShowVideo3()
{
	HideAllVideos();
	
	if (m_nLayout == LAYOUT_AVERAGE){
		m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 4, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);

	}
	else if (m_nLayout == LAYOUT_TEACHER_BIG){
		m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height(), FALSE);
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 2, m_rcVideoArea.Height() / 2, FALSE);
	}

	ShowVideosWnd();
	m_nScreenMode = SCREEN_VIDEO3;
	ShowButtonsNormal();
}
...
void CVideoDlg::ShowVideo4()
{
	...
	if (m_nLayout == LAYOUT_AVERAGE){
		...
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() * 2 / 3, m_rcVideoArea.top, m_rcVideoArea.Width() / 3, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[3]->MoveWindow(0, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width(), m_rcVideoArea.Height() / 2, FALSE);
	}
...
void CVideoDlg::ShowVideo5()
{
	...
	if (m_nLayout == LAYOUT_AVERAGE){
		...
		m_wndShowVideos[0]->MoveWindow(0, m_rcVideoArea.top, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[1]->MoveWindow(m_rcVideoArea.Width() / 4, m_rcVideoArea.top, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[2]->MoveWindow(m_rcVideoArea.Width() / 2, m_rcVideoArea.top, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[3]->MoveWindow(m_rcVideoArea.Width() * 3 / 4, m_rcVideoArea.top, m_rcVideoArea.Width() / 4, m_rcVideoArea.Height() / 2, FALSE);
		m_wndShowVideos[4]->MoveWindow(0, m_rcVideoArea.top + m_rcVideoArea.Height() / 2, m_rcVideoArea.Width(), m_rcVideoArea.Height() / 2, FALSE);
	}

	ShowVideosWnd();
	m_nScreenMode = SCREEN_VIDEO5;
	ShowButtonsNormal();
}
...
void CVideoDlg::ShowVideo6()
{
	...
	ShowVideosWnd();
	m_nScreenMode = SCREEN_VIDEO6;
	ShowButtonsNormal();
}

void CVideoDlg::ShowVideo7()
{
	HideAllVideos();
...
void CVideoDlg::ShowMulti()
{
	...
	int count = m_listWndInfo.GetCount();
...
void CVideoDlg::RebindVideoWnd()
{
	...
	POSITION pos = m_listWndInfo.GetHeadPosition();
	for (int nIndex = 0; nIndex < m_listWndInfo.GetSize(); nIndex++) {
		if (pos != NULL) {
			AGVIDEO_WNDINFO &agvWndInfo = m_listWndInfo.GetNext(pos);
			...
			m_wndVideo[nIndex].ShowWindow(SW_SHOW);
			//m_wndVideo[nIndex].SetRemoteInfo(agvWndInfo.nBitrate, agvWndInfo.nFramerate, agvWndInfo.nWidth, agvWndInfo.nHeight);
			CString strInfo;
			strInfo.Format(_T("setupRemot$eVideo:view=%x, uid=%d\n"), canvas.uid, canvas.view);
	...
	for (int i = 0; i < m_listWndInfo.GetCount(); i++){
		m_wndShowVideos[i] = &m_wndVideo[i];
	}

	showCount = m_listWndInfo.GetCount() + bShowLocalVideo;
	if (bShowLocalVideo){
		if (m_listWndInfo.GetCount() > 0)
			m_wndShowVideos[showCount-1] = &m_wndLocal;
		else
			m_wndShowVideos[0] = &m_wndLocal;
	}
...
LRESULT CVideoDlg::OnShowModeChanged(WPARAM wParam, LPARAM lParam)
{
	if (m_listWndInfo.GetCount() <= 1)
		ShowVideo1();
	else if (m_listWndInfo.GetCount() == 2)
		ShowVideo3();
	else if (m_listWndInfo.GetCount() == 3)
		ShowVideo4();
	else if (m_listWndInfo.GetCount() == 4){
		ShowVideo5();
	}
	else if (m_listWndInfo.GetCount() == 5){
		ShowVideo6();
	}
	else if (m_listWndInfo.GetCount() == 6){
		ShowVideo7();
	}
...
void CVideoDlg::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	if (bShow && GetSafeHwnd() != NULL) {
		...
		if (nRole == 1){
			...
			bShowLocalVideo = oldShowLocalVideo;
			m_btnShowLocal.ShowWindow(SW_SHOW);
		}
		else{
			m_cbxRole.SetCurSel(1);
			m_btnShowLocal.ShowWindow(SW_HIDE);
			oldShowLocalVideo = bShowLocalVideo;
			bShowLocalVideo = FALSE;
		}
	}
}