...
//	m_dlgSetup.MoveWindow(110, 70, 500, 450, TRUE);
	...
	CString strRes;
	strRes.Format(_T("%dx%d %dfps %dkbps"), AgoraGetSetupData()->width, AgoraGetSetupData()->height, AgoraGetSetupData()->framerate, AgoraGetSetupData()->bitrate);
	m_dlgEnterChannel.SetVideoString(strRes);
}
...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	lpAgoraObject->SetVideoProfileEx(AgoraGetSetupData()->width, AgoraGetSetupData()->height, AgoraGetSetupData()->framerate, AgoraGetSetupData()->bitrate);
	lpAgoraObject->SetLocalMirror(FALSE);

	lpAgoraObject->SetEncoderType(AgoraGetSetupData()->encoder_type);
	lpAgoraObject->EnableIntraRequest(AgoraGetSetupData()->intra_request);
	lpAgoraObject->SetFluencyPrecedence(AgoraGetSetupData()->fluency_precedence);
