...
BOOL CAgoraObject::SetVideoProfile(VIDEO_PROFILE_TYPE type, bool swap)
{
	int nRet = m_lpAgoraEngine->setVideoProfile(type, swap);
	videoProfileType = type;
	return nRet == 0 ? TRUE : FALSE;
}

BOOL CAgoraObject::SetVideoProfileEx(int width, int height, int fps, int bitrate)
{
	IRtcEngine2* rtcEngine2 = (IRtcEngine2*)m_lpAgoraEngine; 
	int nRet = rtcEngine2->setVideoProfileEx(width, height, fps, bitrate);
	return nRet == 0 ? TRUE : FALSE;
}
...
void CAgoraObject::SetLocalMirror(BOOL bEnable)
{
	RtcEngineParameters rep(*m_lpAgoraEngine);

	if (bEnable)
		rep.setLocalVideoMirrorMode(VIDEO_MIRROR_MODE_ENABLED);
	else
		rep.setLocalVideoMirrorMode(VIDEO_MIRROR_MODE_DISABLED);
}
...
BOOL CAgoraObject::SetEncoderType(int index)
{
	AParameter apm(*m_lpAgoraEngine);

	int	nRet = apm->setInt("che.hardware_encoding", index);
...
BOOL CAgoraObject::EnableIntraRequest(BOOL bEnable)
{
	AParameter apm(*m_lpAgoraEngine);
	...
	if (bEnable)
		str = "{\"che.video.has_intra_request\":true}";
	else
		str = "{\"che.video.has_intra_request\":false}";

	int nRet = apm->setParameters(str);
...
BOOL CAgoraObject::SetExtentParam(LPCTSTR lpExtendParam)
{
	AParameter apm(*m_lpAgoraEngine);

	if (lpExtendParam == NULL)
		return FALSE;

#ifdef UNICODE
	CHAR szParam[MAX_PATH];

	::WideCharToMultiByte(CP_ACP, 0, lpExtendParam, -1, szParam, MAX_PATH, NULL, NULL);
	int nRet = apm->setParameters(szParam);
#else
	int nRet = apm->setParameters(lpExtendParam);
#endif

	return nRet == 0 ? TRUE : FALSE;
}
BOOL CAgoraObject::SetFluencyPrecedence(bool fp)
{
	RtcEngineParameters rep(*m_lpAgoraEngine);
	return rep.setVideoQualityParameters(!fp);
}
CString CAgoraObject::GetExtentParam(LPCTSTR lpExtendParam)
{
	AParameter	apm(*m_lpAgoraEngine);
	agora::util::AString		astrResult;
	CString		strResult;
	int			nValue = 0;
	...
	::WideCharToMultiByte(CP_ACP, 0, lpExtendParam, -1, szParam, MAX_PATH, NULL, NULL);
	int nRet = apm->getInt(szParam, nValue);
	nRet = apm->getString(szParam, astrResult);

	strResult.Format(_T("result: %s"), astrResult);

	//	::MultiByteToWideChar(CP_ACP, 0, astrResult->c_str(), -1, strResult.GetBuffer(MAX_PATH), MAX_PATH);
	//	strResult.ReleaseBuffer();

#else
	int nRet = apm->setParameters(lpExtendParam, astrResult);
	strResult = astrResult->c_str();
#endif
